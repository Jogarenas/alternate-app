...
// The API key lives in .env.local (local) or Vercel env vars (production).
// NEVER call the AI API directly from the browser.

const proxyCall = async (endpoint, body) => {
...
const analyzeMeal   = ({ mealText, goal, restrictions }) =>
  proxyCall("/api/analyze", { mealText, goal, restrictions });

const fetchInsights = ({ goal, restrictions, analyses }) =>
  proxyCall("/api/insights", { goal, restrictions, analyses });

const fetchCompare  = ({ analysisA, analysisB, goal, restrictions }) =>
  proxyCall("/api/compare", { analysisA, analysisB, goal, restrictions });
...
const saveHistory  = l  => { try { localStorage.setItem(LS_KEY, JSON.stringify(l.slice(0,40))); } catch {} };
const loadDone     = () => { try { return JSON.parse(localStorage.getItem(LS_DONE))||[]; } catch { return []; } };
const saveDone     = d  => { try { localStorage.setItem(LS_DONE, JSON.stringify(d)); } catch {} };
